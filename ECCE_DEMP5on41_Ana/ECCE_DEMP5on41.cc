...
h1_t_cut4_High = new TH1F("t_result_cut4_High", "t dist 15.0 < Q^{2} < 20.0,  t cut, #theta_{n} cut, #theta_{diff} , phi_{diff}, p_{miss} cuts; -t(GeV^{2})", 10, 0, 0.4); // mfek 05/26/2022 - changed binning for 5 on 41
  ...
  for(Int_t A = 0; A < 19; A++){
    ...
    h1_Q2_Resolution[A]=new TH1F(Form("Q2_Resolution_Q2_%i", (A+1)), Form("Q^{2} - Q^{2}_{truth} Dist, %2.1f < Q^{2} < %2.1f; (Q^{2} - Q^{2}_{truth})/Q^{2}_{truth}; Rate (Hz)", Q2BinVal[A], Q2BinVal[A+1]), 200, -100, 100); // added new Q2 resolution plot - mfek 07/21/2022

    h1_W_Resolution[A]=new TH1F(Form("W_Resolution_Q2_%i", (A+1)), Form("W - W_{truth} Dist, %2.1f < W < %2.1f; (W - W_{truth}/W_{truth}); Rate (Hz)", Q2BinVal[A], Q2BinVal[A+1]), 200, -100, 100); // added new W resolution plot - mfek 07/21/2022
  } // mfek 06/21/2022 - changed binning for 5 on 41
...
int ECCE_DEMP5on41::process_event(PHCompositeNode *topNode)
{
  ...
  if (Check_n(topNode) == true && Check_ePi(topNode) == true){ // For event, check if it look like we have an e/pi/n in the event
	...
	if ( Q2 > Q2_low && Q2 < Q2_high){
	  ...
	  if (t_alt < 0.4 && ((nRec4Vect.Theta()*TMath::RadToDeg() > (Thetan_Cent-0.5)) && (nRec4Vect.Theta()*TMath::RadToDeg() < (Thetan_Cent+0.5))) && (abs(nTheta_Diff*TMath::RadToDeg())) < ThetaDiff_Cut && (abs(nPhi_Diff*TMath::RadToDeg())) < PhiDiff_Cut && (nRec4Vect.P() < PmissCutValIP6[B])){ 
	...
	h1_nTheta_tCut->Fill(nRec4Vect.Theta()*TMath::RadToDeg(), wgt);
      ...
      if ( Q2 > Q2_low && Q2 < Q2_high){
	if ( (pmiss4Vect.P() < PmissCutValIP6[B]) && ((pmiss4Vect.Theta()*TMath::RadToDeg() > (Thetan_Cent-0.5)) && (pmiss4Vect.Theta()*TMath::RadToDeg() < (Thetan_Cent+0.5))) && t < 0.4){
	  h1_t_Resolution[B]->Fill(t-t_truth, wgt);
	}
      }
      if ( Q2 > Q2_low && Q2 < Q2_high){
	if ( (n4Vect.P() < PmissCutValIP6[B]) && ((n4Vect.Theta()*TMath::RadToDeg() > (Thetan_Cent-0.5)) && (n4Vect.Theta()*TMath::RadToDeg() < (Thetan_Cent+0.5))) && (abs(nTheta_Diff*TMath::RadToDeg())) < ThetaDiff_Cut && (abs(nPhi_Diff*TMath::RadToDeg())) < PhiDiff_Cut && t_alt_ZDC < 0.4){
	  h1_talt_Resolution_ZDC[B]->Fill(t_alt_ZDC-t_truth, wgt);
	}
      }
      if ( Q2 > Q2_low && Q2 < Q2_high){
	if ( (nRec4Vect.P() < PmissCutValIP6[B]) && ((nRec4Vect.Theta()*TMath::RadToDeg() > (Thetan_Cent-0.5)) && (nRec4Vect.Theta()*TMath::RadToDeg() < (Thetan_Cent+0.5))) && (abs(nTheta_Diff*TMath::RadToDeg())) < ThetaDiff_Cut && (abs(nPhi_Diff*TMath::RadToDeg())) < PhiDiff_Cut && t_alt < 0.4){
	...
	if (t_alt < 0.4){
	  ...
	  if ( (nRec4Vect.P() < PmissCutValIP6[B]) && ((nRec4Vect.Theta()*TMath::RadToDeg() > (Thetan_Cent-0.5)) && (nRec4Vect.Theta()*TMath::RadToDeg() < (Thetan_Cent+0.5))) && (abs(nTheta_Diff*TMath::RadToDeg())) < ThetaDiff_Cut && (abs(nPhi_Diff*TMath::RadToDeg())) < PhiDiff_Cut){
	...
	if ( Q2 > Q2_low && Q2 < Q2_high){
	  ...
	  if (t_alt < 0.4 && ((nRec4Vect.Theta()*TMath::RadToDeg() > (Thetan_Cent-0.5)) && (nRec4Vect.Theta()*TMath::RadToDeg() < (Thetan_Cent+0.5))) && (abs(nTheta_Diff*TMath::RadToDeg())) < ThetaDiff_Cut && (abs(nPhi_Diff*TMath::RadToDeg())) < PhiDiff_Cut && (nRec4Vect.P() < PmissCutValIP8[B])){ 
	    h1_t_cut4_Low->Fill(t_alt, wgt);	  
	  }
	}
      }
	...
	h1_nTheta_tCut->Fill(nRec4Vect.Theta()*TMath::RadToDeg(), wgt);
      ...
      // t resolution plots under different conditions - we treat each different t value as though it is the "true" value of t
      if ( Q2 > Q2_low && Q2 < Q2_high){
	if ( (pmiss4Vect.P() < PmissCutValIP8[B]) && ((pmiss4Vect.Theta()*TMath::RadToDeg() > (Thetan_Cent-0.5)) && (pmiss4Vect.Theta()*TMath::RadToDeg() < (Thetan_Cent+0.5))) && t < 0.4){
	  h1_t_Resolution[B]->Fill(t-t_truth, wgt);
	}
      }
      if ( Q2 > Q2_low && Q2 < Q2_high){
	if ( (n4Vect.P() < PmissCutValIP8[B]) && ((n4Vect.Theta()*TMath::RadToDeg() > (Thetan_Cent-0.5)) && (n4Vect.Theta()*TMath::RadToDeg() < (Thetan_Cent+0.5))) && (abs(nTheta_Diff*TMath::RadToDeg())) < ThetaDiff_Cut && (abs(nPhi_Diff*TMath::RadToDeg())) < PhiDiff_Cut && t_alt_ZDC < 0.4){
	  h1_talt_Resolution_ZDC[B]->Fill(t_alt_ZDC-t_truth, wgt);
	}
      }
      if ( Q2 > Q2_low && Q2 < Q2_high){
	if ( (nRec4Vect.P() < PmissCutValIP8[B]) && ((nRec4Vect.Theta()*TMath::RadToDeg() > (Thetan_Cent-0.5)) && (nRec4Vect.Theta()*TMath::RadToDeg() < (Thetan_Cent+0.5))) && (abs(nTheta_Diff*TMath::RadToDeg())) < ThetaDiff_Cut && (abs(nPhi_Diff*TMath::RadToDeg())) < PhiDiff_Cut && t_alt < 0.4){
	  h1_talt_Resolution_pMiss[B]->Fill(t_alt-t_truth, wgt);
	}
      }
	...
	if (t_alt < 0.4){
	  ...
	  h1_pn_result[B]->Fill(nRec4Vect.P(), wgt);
          cut2Events++; // mfek 05/27/2022 - t cut
	  // Apply other cuts
	  //if ( (pmiss4Vect.P() < PmissCutValIP8[B]) && ((nSmeared4Vect.Theta()*TMath::RadToDeg() > (Thetan_Cent-0.5)) && (nSmeared4Vect.Theta()*TMath::RadToDeg() < (Thetan_Cent+0.5)))){
	  // SJDK - 31/03/22
	  // nRec4Vect is EXACTLY the missing momentum vector for now, so comparing it to the pmiss cut value is fine
	  //if ( (nRec4Vect.P() < PmissCutValIP8[B]) && ((nRec4Vect.Theta()*TMath::RadToDeg() > (Thetan_Cent-0.5)) && (nRec4Vect.Theta()*TMath::RadToDeg() < (Thetan_Cent+0.5)))){ // Old version without theta/phi diff cuts
	  // mfek 05/27/2022 - adding an extra if loop system to count events passing through the cuts
	  if ((nRec4Vect.Theta()*TMath::RadToDeg() > (Thetan_Cent-0.5)) && (nRec4Vect.Theta()*TMath::RadToDeg() < (Thetan_Cent+0.5))) {
	    cut3Events++;
	    if (abs(nTheta_Diff*TMath::RadToDeg()) < ThetaDiff_Cut && (abs(nPhi_Diff*TMath::RadToDeg())) < PhiDiff_Cut) {
	  ...
	  if ( (nRec4Vect.P() < PmissCutValIP8[B]) && ((nRec4Vect.Theta()*TMath::RadToDeg() > (Thetan_Cent-0.5)) && (nRec4Vect.Theta()*TMath::RadToDeg() < (Thetan_Cent+0.5))) && (abs(nTheta_Diff*TMath::RadToDeg())) < ThetaDiff_Cut && (abs(nPhi_Diff*TMath::RadToDeg())) < PhiDiff_Cut){
	    h1_t_cut_result[B]->Fill(t_alt, wgt);
	    h1_Q2_cut_result[B]->Fill(Q2, wgt);
